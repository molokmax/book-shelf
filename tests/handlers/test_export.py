...
def test_handle_export_sends_csv_and_message():
    ...
    with (
        ...
        # Execute the handler
        handle_export_command(vk=fake_vk, user_id=user_id, upload=fake_upload, chat_id=chat_id)
    ...
    fake_upload.document_message.assert_called_once_with(str(mock_path), title="books.csv", peer_id=chat_id)
