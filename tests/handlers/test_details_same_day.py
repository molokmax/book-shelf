...
class FakeVk:
    ...
    def send(self, user_id, message, keyboard, random_id):
        self.sent_messages.append({"user_id": user_id, "message": message, "keyboard": keyboard, "random_id": random_id})
...
def test_details_counts_today_pages():
    ...
    active_states[1] = {"command": "/details", "state": "selecting_book", "data": {"books": ["1"]}}
    # mock services
    with patch("vk_bot.handlers.details.BookService", return_value=FakeBookService([book])):
        with patch("vk_bot.handlers.details.ReadingStatsService") as MockStats:
            ...
            handle_details_step(fake_vk, user_id=1, text="1")
